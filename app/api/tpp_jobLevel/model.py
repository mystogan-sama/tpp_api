...
def check_unit_privilege_read(orm_execute_state):
    check_unit_and_employee_privilege_on_read_db(orm_execute_state, tpp_jobLevel)


@event.listens_for(tpp_jobLevel, 'before_insert')
...
@event.listens_for(tpp_jobLevel, 'before_update')
def check_unit_privilege_delete(mapper, connection, target):
    member_of_list = current_user['member_of_list']
    check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)


@event.listens_for(tpp_jobLevel, 'before_delete')
...
@event.listens_for(tpp_jobLevel, 'after_insert')
...
@event.listens_for(tpp_jobLevel, 'after_update')
...
@event.listens_for(tpp_jobLevel, 'after_delete')
