...
class tpp_cluster_det(db.Model):
    __tablename__ = f'{modelName}'
    id = db.Column(db.BigInteger, primary_key=True, autoincrement=True)
    id_cluster = db.Column(db.Integer, db.ForeignKey("tpp_cluster.id"), nullable=True)
...
#     check_unit_and_employee_privilege_on_read_db(orm_execute_state, tpp_cluster_det)
#
#
# @event.listens_for(tpp_cluster_det, 'before_insert')
...
#
#
# @event.listens_for(tpp_cluster_det, 'before_update')
# def check_unit_privilege_delete(mapper, connection, target):
#     member_of_list = current_user['member_of_list']
#     check_unit_privilege_on_changes_db(mapper, connection, target, member_of_list)
#
#
# @event.listens_for(tpp_cluster_det, 'before_delete')
...
# @event.listens_for(tpp_cluster_det, 'after_insert')
...
#
#
# @event.listens_for(tpp_cluster_det, 'after_update')
...
#
#
# @event.listens_for(tpp_cluster_det, 'after_delete')
