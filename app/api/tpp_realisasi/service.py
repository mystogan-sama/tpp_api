    ...
    def getSummary(args):
        try:
            ...
            select_query = db.session.query(
                tpp_realisasi.id_unit,
                func.sum(tpp_realisasi_det.kriteria_pagu).label("total_kriteria_pagu")
            ).join(tpp_realisasi_det, tpp_realisasi.id == tpp_realisasi_det.id_pagu)

            # Tambahkan filter opsional
            if id_unit:
                select_query = select_query.filter(tpp_realisasi.id_unit == id_unit)
            if asn is not None:
                select_query = select_query.filter(tpp_realisasi.asn == asn)

            # Group by untuk menghindari hasil duplicate per unit
            select_query = select_query.group_by(tpp_realisasi.id_unit)
