...
from .doc import doc
from .model import tpp_realisasi
...
class List(Resource):
    ...
    def post(self):
        try:
            ...
            if bulan is None or tahun is None or asn is None:
                return {"message": "Kolom bulan, tahun, dan asn wajib diisi"}, 400

            # Ambil semua record dengan bulan & tahun yang sama
            existing_records = tpp_realisasi.query.filter_by(id_unit=id_unit, bulan=bulan, tahun=tahun).all()
            existing_asn_values = [r.asn for r in existing_records]

            # Jika sudah ada data untuk asn yang sama, tolak penambahan
            if asn in existing_asn_values:
                bulan_str = existing_records[0].bulan_str if existing_records else bulan
                unit_name = existing_records[0].unit_name if existing_records else id_unit
                jenis = "PNS" if asn == 1 else "PPPK"
                return {
                    "message": f"Tidak dapat menambah data. Realisasi untuk SKPD '{unit_name}' {jenis} pada Bulan '{bulan_str}' Tahun {tahun} sudah terdaftar."
                }, 400

            # Jika sudah ada dua-duanya (PNS dan PPPK), tolak semua
            if set(existing_asn_values) == {0, 1}:
                bulan_str = existing_records[0].bulan_str if existing_records else bulan
                return {
                    "message": f"Tidak dapat menambah data. Realisasi untuk PNS dan PPPK pada Bulan '{bulan_str}' Tahun {tahun} sudah terdaftar."
                }, 400

            # Jika lolos validasi, lanjutkan proses insert
